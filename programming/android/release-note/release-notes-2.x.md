...
- Added an interface [`DCECameraStateListener`](../auxiliary-api/interface-dcecamerastatelistener.md) to receive a callback when the camera state changes. You can add the interface via a new method [`setCameraStateListener`](../primary-api/camera-enhancer.md#setcamerastatelistener).
